...
def teachers(request):
    ...
    for teacher in teachers:
        ...
        # teacher.gains = Courses.objects.filter(user_id=teacher.id).first().gains
...
def students(request):
    ...
    if category == 0:
        studentList = User.objects.filter(group_id=2).filter(Q(first_name__contains=search) | Q(last_name__contains=search) | Q(email__contains=search))
    else:
        studentListTmp = User.objects.filter(group_id=2).filter(
...
@csrf_exempt
def watchStudent(request):
    id = request.POST.get('id')
    course_id = request.POST.get('course_id')

    records = []
    video_caches = Cache.objects.all();
    # getting the registered courses with student
    # registered_courses = Student_register_courses.objects.filter(student_id_id=id)

    # cached_courses = [];
    cached_course = {}
    for video_cache in video_caches:
        stu_id = video_cache.key.split('-')[0]
        cou_id = int(video_cache.key.split('-')[1])
        if stu_id == id and cou_id == int(course_id):
            cached_course['cid'] = cou_id
            cached_course['checkList'] = len(list(filter(lambda x: x == 1, json.loads(video_cache.cache_str)['checkList'])))
            # cached_courses.append(temp)
            break
    # for registered_course in registered_courses:
    course = Courses.objects.get(pk=course_id)
    course_sections = Sections.objects.filter(course_id=course_id, type="video")

    # for cached_course in cached_courses:
    if len(cached_course):
        for section in course_sections:
            section_videos = VideoUploads.objects.filter(section_id=section.id)
            idx = 0;
            for video in section_videos:
                ...
                item['checked'] = 1
                if idx > cached_course['checkList']:
                    item['checked'] = 0
...
def test(request):
    ...
    user = request.user
    sender_id = User.objects.filter(email=user).first().id
    # sender_id = 35
    print('------------I am -----------------',sender_id)
    host_name = Option.objects.all().first().oval
    return render(request, 'test.html', {'video_list': videos, 'search': search, 'menu_no':16, 'sender_id': sender_id, 'host_name': host_name})
...
def setApprove(request):
    ...
    text += f'<td style="padding-right: 0px;padding-left: 0px;" align="center"><img align="center" border="0" src="http://{domain}/static/assets/img/favicon.png" alt="Image" title="Image" style="outline: none;text-decoration: none;-ms-interpolation-mode: bicubic;clear: both;display: inline-block !important;border: none;height: auto;float: none;width: 100%;max-width: 80px;" width="80">'
    ...
    msg = EmailMultiAlternatives(subject, '...', 'support@booctep.com', [user.email])
